...
BROAD_RULES: List[KeywordRule] = [
    ...
    KeywordRule(
        ...
        keywords=[
            ...
            "enquêteurs téléphoniques",
            "enqueteur telephonique",
            "enqueteurs telephoniques",
            "prise de rdv",
            "prise de rendez",
            "rdv",
        ],
...
# Current operating mode: scrape everything and consider broad target roles relevant
# for routing, scoring, and alerts. That includes SALES, TECH, DATA, and AI,
# while still respecting hard block patterns.
def is_relevant(text: str, rules: Iterable[KeywordRule] = BROAD_RULES) -> bool:
    if is_blocked(text):
        return False
    labels = set(match_labels(text, rules=rules))
    return bool(labels.intersection({"SALES", "TECH", "DATA", "AI"}))
