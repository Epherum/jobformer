...
def ensure_jobs_header(cfg: SheetsConfig) -> None:
    values = [["source","labels","title","company","location","date_added","url","decision","score","reason"]]
    _run_gog(["gog","sheets","update",cfg.sheet_id,f"{cfg.tab}!A1:J1","--account",cfg.account,"--values-json",json.dumps(values, ensure_ascii=False),"--input","USER_ENTERED"])
...
def update_job_scores(cfg: SheetsConfig, updates: Sequence[dict]) -> int:
    ...
    def _flush_block(start: int, end: int) -> None:
        ...
        _run_gog(["gog","sheets","update",cfg.sheet_id,f"{cfg.tab}!I{start}:J{end}","--account",cfg.account,"--values-json",json.dumps(values, ensure_ascii=False),"--input","USER_ENTERED"])
